...
if(! function_exists('thumb_asset')){
    function thumb_asset($path, $width, $height)
    {
        $info = pathinfo($path);

        $extension = strtolower($info['extension']);
        $file_name = strtolower($info['filename']);
        $dirname = strtolower($info['dirname']).'/thumb';
        $dest = 'cache/'.$dirname . '/' . $file_name.'-'.$width.'x'.$height.'.'.$extension;
        if(!file_exists($dest)){
            if(!is_dir('cache/'.$dirname)){
                mkdir('cache/'.$dirname, 0755, true);
            }
...
if(! function_exists('resize_asset')){
    function resize_asset($path, $width, $height)
    {
        $info = pathinfo($path);

        $extension = strtolower($info['extension']);
        $file_name = strtolower($info['filename']);
        $dirname = strtolower($info['dirname']);
        $dest = 'cache/'.$dirname . '/' . $file_name.'-'.$width.'x'.$height.'.'.$extension;
...
if(! function_exists('fit_asset')){
    function fit_asset($path, $width, $height)
    {
        $info = pathinfo($path);
...
if (!function_exists('getHref') ) {
    function getHref($link) {
        ...
        } else if ($link["type"] == 'dinning') {
            $href = route('front.dinning');
        } else if ($link["type"] == 'special_offers') {
            $href = route('front.special_offers');
