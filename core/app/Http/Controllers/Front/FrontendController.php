...
class FrontendController extends Controller
{
    ...
    public function product(Request $request)
    {
        ...
        if ($version == 'amaya') {
            ...
            return view('front.amaya.product.product', $data);
        }

        return view('front.index.product', $data);
    ...
    public function index()
    {
        ...
        } elseif ($version == 'amaya') {
            $data['testimonials'] = Testimonial::where('language_id', $lang_id)->orderBy('serial_number', 'ASC')->get();
            $data['special_href'] = getHref(['type' => 'special_offers']);
            $data['special_offers'] = SpecialOffer::where('feature', 1)->where('language_id', $currentLang->id)->orderBy('serial_number', 'ASC')->get();
            $data['gcategories'] = GalleryCategory::where('language_id', $lang_id)->where('status', 1)
                ->orderBy('serial_number', 'ASC')->get();

            $data['galleries']['image'] = Gallery::where('language_id', $lang_id)->where('type', 'image')->orderBy('serial_number', 'ASC')->get();
            $data['galleries']['video'] = Gallery::where('language_id', $lang_id)->where('type', 'video')->orderBy('serial_number', 'ASC')->get();

            return view('front.amaya.index', $data);
        }
        // Thêm vào để chạy
        else if ($version == 'protocol') {
            $data['testimonials'] = Testimonial::where('language_id', $lang_id)->orderBy('serial_number', 'ASC')->get();
            $data['special_href'] = getHref(['type' => 'special_offers']);
            $data['special_offers'] = SpecialOffer::where('feature', 1)->where('language_id', $currentLang->id)->orderBy('serial_number', 'ASC')->get();
            ...
            //get home categories
            //$home_categories = Category::where('is_home',1)->get();
            //foreach($home_categories as &$category){
            //   $home_products = Product::where('category_id',$category->id)->where('is_home',1)->get();
            //  $category->products = $home_products;
            //}
            //each category -> get home products of that category
            //
            //$data['home_product_tabs'] = $home_categories;
            // Product in  Home
            $home_categories = Pcategory::where([['language_id', $lang_id],['is_home', 1]])->with(['products'=>function($query) use($lang_id){
                $query->where([['language_id', $lang_id],['is_home', 1]]);
            }])->orderBy('name','asc')->get();
            $data['home_product_tabs'] = $home_categories;
            
            return view('front.protocol.index', $data);
    ...
    public function services(Request $request)
    {
        ...
        if ($version == 'gym') {
            ...
            $data['version'] = $version;
            $data['services_indoor'] = Service::where('language_id', $currentLang->id)->where('type', 'indoor')->where('feature', 1)->get();
            $data['services_outdoor'] = Service::where('language_id', $currentLang->id)->where('type', 'outdoor')->where('feature', 1)->get();
            $data['testimonials'] = Testimonial::where('language_id', $currentLang->id)->orderBy('serial_number', 'ASC')->get();
            return view('front.amaya.service.services', $data);
        }
    }
    ...
    public function special_offers(Request $request)
    {
        ...
        $data['version'] = $be->theme_version;

        return view('front.amaya.special_offer.special_offers', $data);
    ...
    public function dinning(Request $request)
    {
        ...
        $bex = BasicExtra::where('language_id', $currentLang->id)->first();
        $sliderImages = [];
        $images = unserialize($bex->dinning_page_slider);
        foreach ($images as $key => $image) {
            $sliderImages[] = resize_asset("assets/front/img/dinnings/sliders/$image",null,413);
        }
        $data['sliders'] = $sliderImages;

        $sliderBoxImages = [];
        $images_box = unserialize($bex->dinning_page_slider_box);
        foreach ($images_box as $key1 => $image1) {
            $sliderBoxImages[] = url("assets/front/img/dinnings/sliders/$image1");
        }
        $data['sliders_box'] = $sliderBoxImages;
        $data['sliders'] = $sliderImages;
        $data['bex'] = $bex;
        $data['dinnings'] = Dinning::where('feature', 1)->where('language_id', $currentLang->id)->orderBy('serial_number', 'ASC')->get();

        $data['version'] = $be->theme_version;

        return view('front.amaya.dinning.dinning', $data);

    }

    public function create_thumb(Request $request){
        $path = $request->path;
        $img = $request->img;
        $result = thumb_asset($path.'/'.$img,120,null);
        $extImage = strtolower(pathinfo($img, PATHINFO_EXTENSION));
        $mime = 'image/jpeg';
        switch ($extImage){
            case 'jpg':
            case 'jpeg':
    ...
    public function dinning_menu($slug)
    {
        ...
        $data['dinning'] = Dinning::where('slug', $slug)->firstOrFail();
        ...
        return view('front.amaya.dinning.dinning_menu', $data);
    }
    ...
    public function packages()
    {
      ...
      if ($version == 'gym') {
        ...
        return view('front.packages', $data);
          ...
          $data['version'] = $version;
        return view('front.amaya.package.packages', $data);
      }
    }
    ...
    public function gallery()
    {
      ...
      $data['galleries'] = Gallery::with('galleryImgCategory')->where('language_id', $lang_id)->where('type', 'image')->orderBy('serial_number', 'ASC')->get();
      ...
      $data['version'] = $version;
        ...
        if ($version == 'amaya') {
            ...
            $data['videos'] = Gallery::where('language_id', $lang_id)->where('type', 'video')->orderBy('serial_number', 'ASC')->get();

            return view('front.amaya.gallery', $data);
        }
    ...
    public function faq()
    {
      ...
      $data['version'] = $version;
        ...
        if ($version == 'amaya') {
            return view('front.amaya.faq', $data);
        }
    ...
    public function dynamicPage($slug)
    {
        ...
        if ($version == 'amaya') {
            if($bex->custom_page_pagebuilder == 1) {
                return view('front.amaya.custom_page.dynamic', $data);
            } else {
                return view('front.amaya.custom_page.dynamic1', $data);
            }
        }
        if($bex->custom_page_pagebuilder == 1) {
    ...
    public function changeLanguage($lang, Request $request)
    {
        ...
        if($request->redirect){
            ...
            $c_slug = $request->slug;
            if(strpos($c_slug, '/') !== false){
                $c_slug = explode('/', $c_slug);
                $c_slug = end($c_slug);
            }

            if($route == 'front.dynamicPage'){
                $page = Page::where('slug', $c_slug)->first();

                $page_assoc = Page::where('assoc_id',$page->assoc_id)->where('language_id', $language->id)->first();
                return redirect()->route($route, $page_assoc->slug);
            }
            if($route == 'front.dinning_menu'){
                $dinning = Dinning::where('slug', $c_slug)->firstOrFail();

                $dinning_assoc = Dinning::where('assoc_id', $dinning->assoc_id)->where('language_id', $language->id)->firstOrFail();

                if($dinning_assoc->count() > 0){
                    return redirect()->route($route, $dinning_assoc->slug);
                }else {
