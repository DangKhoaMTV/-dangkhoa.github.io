...
class PackageController extends Controller
{
    public function index(Request $request)
    {
        ...
        foreach ($languages as $lang) {
            $data['packages'][$lang->code] = Package::where('language_id', $lang->id)->orderBy('id', 'DESC')->get();
            $data['categories'][$lang->code] = PackageCategory::where('language_id', $lang->id)
                ->where('status', 1)
                ->get();
        }
    ...
    public function edit(Request $request, $id)
    {
        ...
        $current_lang = Language::where('id', $package->language_id)->first();
        $languages = Language::orderBy('is_default', 'DESC')->get();
        foreach ($languages as $lang) {
            if ($current_lang->id == $lang->id) {
                $data['package'][$lang->code] = $package;
            } else {
                $data['package'][$lang->code] = $package->assoc_id > 0 ? Package::where('language_id', $lang->id)->where('assoc_id', $package->assoc_id)->first() : null;
            }
            if ($data['package'][$lang->code] == null) {
                $data['package'][$lang->code] = new Package();
                $data['pcates'][$lang->code] = Package::where('language_id', $lang->id)->get();
            }
            $data['categories'][$lang->code] = PackageCategory::where('language_id', $lang->id)
                ->where('status', 1)
                ->get();
        ...
        $abe = BasicExtended::where('language_id', $package->language_id)->first();
        $abx = BasicExtra::select('base_currency_text')->where('language_id', $package->language_id)->first();
    ...
    public function preview_modal(Request $request, $id)
    {
        $package = Package::findOrFail($id);
        $current_lang = Language::where('id', $package->language_id)->first();
        $languages = Language::orderBy('is_default', 'DESC')->get();
        ...
        return view('admin.package.preview-modal', $data);
    }
    ...
    public function store(Request $request)
    {
        ...
        foreach ($languages as $lang) {
            $rules = [
                'title_' . $lang->code => 'required|max:40',
                'price_' . $lang->code => 'required|numeric',
                'description_' . $lang->code => 'required',
            ...
            if ($bex->recurring_billing == 0) {
                $rules['order_status_' . $lang->code] = 'required';
                $rules['link_' . $lang->code] = 'required_if:order_status,2';
            } else {
                $rules['duration_' . $lang->code] = 'required';
            }
        ...
        foreach ($languages as $lang) {
            $package = new Package;
            $package->language_id = $lang->id;
            $package->title = $request->{'title_' . $lang->code};
            if ($version == 'lawyer') {
            ...
            $package->price = $request->{'price_' . $lang->code};
            $package->serial_number = $request->{'serial_number_' . $lang->code};
            $package->meta_keywords = $request->{'meta_keywords_' . $lang->code};
            $package->meta_description = $request->{'meta_description_' . $lang->code};
            $package->description = str_replace(url('/') . '/assets/front/img/', "{base_url}/assets/front/img/", $request->{'description_' . $lang->code});

            if ($bex->recurring_billing == 0) {
                $package->order_status = $request->{'order_status_' . $lang->code};
                if ($request->{'order_status_' . $lang->code} == 2) {
                    $package->link = $request->{'link_' . $lang->code};
                }
            } else {
                $package->duration = $request->{'duration_' . $lang->code};
            }


            if ($version == 'cleaning') {
                $package->color = $request->{'color_' . $lang->code};
            }

            $package->category_id = $request->{'category_id_' . $lang->code};
            $package->save();
            ...
            $saved_ids[] = $package->id;
        }
        foreach ($saved_ids as $saved_id) {
            $package = Package::findOrFail($saved_id);
            $package->assoc_id = $assoc_id;
            $package->save();
        }
    ...
    public function update(Request $request)
    {
        ...
        $languages = Language::orderBy('is_default', 'DESC')->get();
                ...
                $saved_ids = [];
                foreach ($languages as $lang) {
                    if ($request->filled('package_id_' . $lang->code) || !$request->filled('package_assoc_id_' . $lang->code)) {//Validation
        ...
        foreach ($languages as $lang) {
            if ($request->filled('package_id_' . $lang->code)) {//update
                $package = Package::findOrFail($request->{'package_id_' . $lang->code});
                $package->title = $request->{'title_' . $lang->code};
                ...
                if (!$request->filled('package_assoc_id_' . $lang->code)) {//create
                    $package = new Package;
                    $package->title = $request->{'title_' . $lang->code};
                    ...
                    $saved_ids[] = $request->{'package_assoc_id_' . $lang->code};
                }
            }
        }
        foreach ($saved_ids as $saved_id) {
    ...
    {
        $_package = Package::findOrFail($request->package_id);
        if($_package->assoc_id > 0) {
            $packages = Package::where('assoc_id', $_package->assoc_id)->get();
            ...
            // if the package has any currently active subscription / subscription requests, then it cannot be deleted
            if ($_package->current_subscriptions()->where('status', 1)->count() > 0 || $_package->pending_subscriptions()->count() > 0) {
                Session::flash('warning', 'Please delete the active subscriptions & subscription requests of this package first');
                return back();
            }
            $_package->current_subscriptions()->delete();
            $_package->next_subscriptions()->delete();
            $_package->pending_subscriptions()->delete();

            @unlink('assets/front/img/packages/' . $_package->image);
    ...
    public function bulkDelete(Request $request)
    {
        ...
        foreach ($ids as $id) {
            $_package = Package::findOrFail($id);
            if ($_package->assoc_id > 0) {
                $packages = Package::where('assoc_id', $_package->assoc_id)->get();
                foreach ($packages as $package) {
                    // if the package has any currently active subscription / subscription requests, then it cannot be deleted
...
// if the package has any currently active subscription / subscription requests, then it cannot be deleted
                if ($_package->current_subscriptions()->where('status', 1)->count() > 0 || $_package->pending_subscriptions()->count() > 0) {
                    Session::flash('warning', 'Please delete the active subscriptions & subscription requests of ' . $_package->title . ' package first');
                    return "success";
                }
