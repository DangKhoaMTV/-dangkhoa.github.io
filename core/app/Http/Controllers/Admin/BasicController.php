...
class BasicController extends Controller
{
    ...
    public function updatelogo(Request $request)
    {
        ...
        $extAdminLogo = pathinfo($admin_logo, PATHINFO_EXTENSION);
        $extMobileLogo = pathinfo($mobile_logo, PATHINFO_EXTENSION);
       ...
       if ($request->filled('admin_logo')) {
            $rules['admin_logo'] = [
                function ($attribute, $value, $fail) use ($extAdminLogo, $allowedExts) {
                    if (!in_array($extAdminLogo, $allowedExts)) {
                        return $fail("Only png, jpg, jpeg, svg image is allowed");
                    }
                }
            ];
        ...
        if ($request->filled('mobile_logo')) {
            $rules['mobile_logo'] = [
                function ($attribute, $value, $fail) use ($extMobileLogo, $allowedExts) {
                    if (!in_array($extMobileLogo, $allowedExts)) {
                        return $fail("Only png, jpg, jpeg, svg image is allowed");
                    }
                }
            ];
        ...
        if ($request->filled('admin_logo')) {
            ...
            foreach ($bss as $key => $bs) {
                @unlink('assets/front/img/' . $bs->admin_logo);
                $filename = uniqid() .'.'. $extAdminLogo;
                @copy($admin_logo, 'assets/front/img/' . $filename);

                $bs->admin_logo = $filename;
                $bs->save();
            }
        ...
        if ($request->filled('mobile_logo')) {
            ...
            foreach ($bss as $key => $bs) {
                @unlink('assets/front/img/' . $bs->mobile_logo);
                $filename = uniqid() .'.'. $extMobileLogo;
                @copy($mobile_logo, 'assets/front/img/' . $filename);

                $bs->mobile_logo = $filename;
                $bs->save();
            }
    ...
    public function updateseo(Request $request, $langid)
    {
        ...
        $be->dinning_meta_keywords = $request->dinning_meta_keywords;
        $be->dinning_meta_description = $request->dinning_meta_description;
        $be->special_offers_meta_keywords = $request->special_offers_meta_keywords;
        $be->special_offers_meta_description = $request->special_offers_meta_description;
    ...
    public function updateheading(Request $request, $langid)
    {
        $request->validate([
            ...
            'dinning_title' => 'nullable|max:30',
            'dinning_subtitle' => 'nullable|max:40',
            'special_offer_title' => 'nullable|max:30',
            'special_offer_subtitle' => 'nullable|max:40',
        ...
        $bs->dinning_title = $request->dinning_title;
        $bs->dinning_subtitle = $request->dinning_subtitle;
        $bs->dinning_details_title = $request->dinning_details_title;
        $bs->special_offer_title = $request->special_offer_title;
        $bs->special_offer_subtitle = $request->special_offer_subtitle;
        $bs->special_offer_details_title = $request->special_offer_details_title;
    ...
    public function updatesections(Request $request)
    {
        ...
        foreach ($bss as $key => $bs) {
            ...
            if ($be->theme_version != 'ecommerce') {
                $bs->special_offer_section = $request->special_offer_section;
