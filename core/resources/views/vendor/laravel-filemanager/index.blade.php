...
<body>
    <nav class="navbar sticky-top navbar-expand-lg navbar-dark" id="nav">
        ...
        <div class="ml-auto px-2">
            <a class="navbar-link d-none" id="select_all_toggle">
                <i class="fa fa-check-double fa-fw"></i>
                <span class="d-none d-lg-inline">{{ trans('laravel-filemanager::lfm.menu-select-all') }}</span>
            </a>
        </div>
        <div class="px-2">
        ...
        // {
            ...
            <script>
               ...
               if (urlParams.get('service')) {
                    let id = urlParams.get('service');
                    $.get("{{url('/')}}" + "/admin/service/" + id + "/images", function(datas){
               ...
               if (urlParams.get('dinning')) {
                   let id = urlParams.get('dinning');
                    $.get("{{url('/')}}" + "/admin/dinning/" + id + "/images", function(datas){
               ...
               if (urlParams.get('dinning_box')) {
                   let id = urlParams.get('dinning_box');
                    $.get("{{url('/')}}" + "/admin/dinning/" + id + "/images_box", function(datas){
                ...
                function prevImages() {
                    ...
                    if (sliders.length > 0) {
                        sliders.forEach(function(slider, index) {
                            var file_paths = slider.split('/');
                            var file_name = file_paths.pop();
                            var file_path = file_paths.join("/");
                            var thumb = file_path+'/thumbs/'+file_name;
                            imagesDiv += `<div class="thumb-preview mr-2 mb-2">
                                ...
                                <img src="${thumb}" data-full="${slider}" alt="Slider Image">
                            </div>`;
                    ...
                    $("#sliderThumbs" + serial, parent.document).sortable({
                        stop: function( event, ui ) {
                            sliders = $.map( $("#sliderThumbs" + serial, parent.document).find('.thumb-preview img'), function( val, i ) {
                                return $(val).data('full');
                            });
                            prevImages();
                        }
                    });
