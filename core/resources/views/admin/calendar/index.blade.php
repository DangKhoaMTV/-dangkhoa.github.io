                    ...
                    <tbody>
                      @foreach ($events as $key => $event)
                        <tr>
                          ...
                          <td>
                            <a class="btn btn-secondary btn-sm editbtn_url" href="#editModal" data-url="{{route('admin.calendar.edit', $event->id) . '?language=' . request()->input('language')}}" data-toggle="modal" data-event_id="{{$event->id}}" data-title="{{$event->title}}" data-start_date="{{$event->start_date}}" data-end_date="{{$event->end_date}}">
                  ...
                  <div class="tab-content">
                      @foreach ($langs as $lang)
                        ...
                        <div class="form-group">
                          ...
                          <input name="title_{{$lang->code}}" class="form-control" placeholder="Enter Title" type="text" value="">
                          <p id="errtitle_{{$lang->code}}" class="mb-0 text-danger em"></p>
                        </div>
                        <div class="form-group">
                          <label for="">Event Period **</label>
                          <input type="text" name="datetimes_{{$lang->code}}" class="form-control ltr" placeholder="Enter Event Period" autocomplete="off"/>
                          <input type="hidden" id="start_date_{{$lang->code}}" name="start_date_{{$lang->code}}" value="">
                          <input type="hidden" id="end_date_{{$lang->code}}" name="end_date_{{$lang->code}}" value="">
                          <p id="errstart_date_{{$lang->code}}" class="mb-0 text-danger em"></p>
                          <p id="errend_date_{{$lang->code}}" class="mb-0 text-danger em"></p>
                        </div>
                          </div>
                        @endforeach
                  </div>
...
<script type="text/javascript">
    function init_daterange(){
        ...
        $('input[name="datetimes_{{$lang->code}}"]').daterangepicker({
        ...
        $('input[name="datetimes_{{$lang->code}}"]').on('apply.daterangepicker', function(ev, picker) {
            $(this).val(picker.startDate.format('MM/DD/YYYY hh:mm A') + ' - ' + picker.endDate.format('MM/DD/YYYY hh:mm A'));
            $("#start_date_{{$lang->code}}").val(picker.startDate.format('MM/DD/YYYY hh:mm A'));
            $("#end_date_{{$lang->code}}").val(picker.endDate.format('MM/DD/YYYY hh:mm A'));
        });

        $('input[name="datetimes_{{$lang->code}}"]').on('cancel.daterangepicker', function(ev, picker) {
            $(this).val('');
            $("#start_date_{{$lang->code}}").val('');
            $("#end_date_{{$lang->code}}").val('');
        });
        @endforeach
    }
    (function () {
