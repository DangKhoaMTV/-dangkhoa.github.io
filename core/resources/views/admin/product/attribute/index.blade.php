...
@section('content')
    <div class="page-header">
        <h4 class="page-title">Product Attribute</h4>
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <div class="row">
                        <div class="col-lg-4">
                            <div class="card-title d-inline-block">Product Attribute</div>
                        </div>
                        <div class="col-lg-3">
                            @if (!empty($langs))
                                    ...
                                    <table class="table table-striped mt-3">
                                        ...
                                        @foreach ($attributes as $key => $attribute)
                                            <tr>
                                                ...
                                                <td>
                                                    @if($attribute->icon_type=='image')
                                                    <img style="background: #fff;width: 48px" src="{{ asset('assets/front/img/product_attribute/'.$attribute->icon) }}"/>
                                                    @else
                                                        <i style="background-color: #fff;color:#000;padding:3px;font-size: 32px" class="{{$attribute->icon}}"></i>
                                                    @endif
                                                </td>
                                                ...
                                                <td>
                                                    <a class="btn btn-secondary btn-sm editbtn_url"
                                                       href="{{route('admin.product.attribute_edit', $attribute->id) . '?language=' . request()->input('language')}}"
                                                       data-url="{{route('admin.product.attribute_edit', $attribute->id) . '?language=' . request()->input('language')}}"
                                ...
                                <span class="btn-label">
                                    <i class="fas fa-edit"></i>
                                                    ...
                                                    <form class="d-inline-block deleteform"
                                                          action="{{route('admin.product.attribute_delete')}}"
                                                          method="post">
                                                        @csrf
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLongTitle">Add Product Attribute</h5>
                ...
                <div class="modal-body">
                    ...
                    @endif
                    <form id="ajaxForm" action="{{route('admin.product.attribute_store')}}" method="post">
                        @csrf
                        @if (!empty($langs))
                                ...
                                @foreach ($langs as $lang)
                                    <div class="tab-pane container {{$lang->code == request()->input('language') ? 'active' : ''}}"
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="icon_type_{{$lang->code}}" class="form-control" id="attribute_type_{{$lang->code}}">
                                        ...
                                        {{-- Image Part --}}
                                        <div class="form-group" id="attribute_image_{{$lang->code}}">
                                        ...
                                        <div class="form-group" id="attribute_icon_{{$lang->code}}" style="display: none;">
                                            <label for="">Icon **</label>
                                            <div class="btn-group d-block">
                                                <button type="button" class="btn btn-primary iconpicker-component"><i class=""></i></button>
                                                <button type="button" class="icp icp-dd btn btn-primary dropdown-toggle"
                                                        data-selected="fa-car" data-toggle="dropdown">
                                                </button>
                                                <div class="dropdown-menu"></div>
                                            </div>
                                            <input id="inputIcon_1{{$lang->code}}" type="hidden" disabled name="icon_{{$lang->code}}" value="">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <p class="text-warning"><small>The higher the serial number is, the later the
                                                    product attribute will be shown.</small></p>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
    ...
    <div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle"
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLongTitle">Edit Product Attribute</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
...
@section('scripts')
    <script>
        (function () {
            ...
            @foreach ($langs as $lang)
            $("#attribute_type_{{$lang->code}}").on('change', function () {
                let type = $(this).val();
                if (type == 'image') {
                    $("#attribute_icon_{{$lang->code}} input").attr('disabled', true);
                    $("#attribute_icon_{{$lang->code}}").hide();
                    $("#attribute_image_{{$lang->code}}").show();
                    $("#attribute_image_{{$lang->code}} input").removeAttr('disabled');
                } else {
                    $("#attribute_image_{{$lang->code}} input").attr('disabled', true);
                    $("#attribute_image_{{$lang->code}}").hide();
                    $("#attribute_icon_{{$lang->code}}").show();
                    $("#attribute_icon_{{$lang->code}} input").removeAttr('disabled');
                }
            });
            @endforeach
