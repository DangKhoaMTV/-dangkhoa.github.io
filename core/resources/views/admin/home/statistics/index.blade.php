                        ...
                        <form id="backgroundForm" action="{{route('admin.statistics.upload', $lang_id)}}" method="POST">
                            ...
                            <div class="form-group">
                                <div class="thumb-preview" id="thumbPreview1{{$lang->id}}">
                                    <label for="chooseImage1{{$lang->id}}">
                                        <img src="{{asset('assets/front/img/' . $abe[$lang->code]->statistics_bg)}}" alt="Image">
                                    </label>
                                </div>
                                <br>
                                <br>
                                ...
                                <input id="fileInput1{{$lang->id}}" type="hidden" name="background_image_{{$lang->code}}">
                                ...
                                @if ($errors->has('background_image_'.$lang->code))
                                <p class="text-danger mb-0">{{$errors->first('background_image_'.$lang->code)}}</p>
                                @endif
                    ...
                    <tbody>
                      @foreach ($statistics as $key => $statistic)
                        <tr>
                          ...
                          <td>
                            <a class="btn btn-secondary btn-sm editbtn_url" data-url="{{route('admin.statistics.edit-modal', $statistic->id) . '?language=' . request()->input('language')}}" href="{{route('admin.statistics.edit', $statistic->id) . '?language=' . request()->input('language')}}" data-toggle="modal" data-target="#editModal">
  ...
  <script>
    $(document).ready(function() {
        $('.icp').on('iconpickerSelected', function(event){
            $("#inputIcon1").val($(".iconpicker-component").find('i').attr('class'));
        });
