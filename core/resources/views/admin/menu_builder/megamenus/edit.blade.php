                ...
                <form action="{{route('admin.megamenu.update')}}" id="megaMenuForm" method="POST">
                                ...
                                <div class="selectgroup selectgroup-pills">
                                    @foreach ($items as $item)
                                        <label class="selectgroup-item">
                                            <input type="checkbox" name="items_{{$lang->code}}[]" value="{!! json_encode([$cat->id, $item->id]) !!}" class="selectgroup-input" {{array_key_exists("$cat->id", $mmenus[$lang->code]) && in_array($item->id, $mmenus[$lang->code]["$cat->id"]) ? 'checked' : ''}}>
                    ...
                    @elseif ((!serviceCategory() && $type == 'services') || (!serviceCategory() && $type == 'portfolios') || ($type == 'causes'))
                        <div class="form-group">
                            ...
                            <br>
                            <div class="selectgroup selectgroup-pills">
                                @foreach ($items[$lang->code] as $item)
                                    <label class="selectgroup-item">
                                        <input type="checkbox" name="items_{{$lang->code}}[]" value="{{$item->id}}" class="selectgroup-input" {{in_array($item->id, $mmenus[$lang->code]) ? 'checked' : ''}}>
                                        <span class="selectgroup-button">{{strlen($item->title) > 30 ? mb_substr($item->title,0,30,'utf-8') . '...' : $item->title}}</span>
                                    </label>
                                @endforeach
                            </div>
