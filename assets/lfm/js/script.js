...
var show_list = localStorage.getItem('lfm_show_list')??'';
var sort_type = localStorage.getItem('lfm_sort_type')??'alphabetic';
var multi_selection_enabled = localStorage.getItem('lfm_multi_selection_enabled')=='true'?true:false;
var selected = [];
var items = [];
  ...
  sortings.forEach(function (sort) {
    $('#nav-buttons .dropdown-menu').append(
      $('<a>').addClass('dropdown-item').attr('data-sortby', sort.by)
        ...
        .click(function() {
          sort_type = sort.by;
          localStorage.setItem('lfm_sort_type',sort_type);
          loadItems();
        })
    );
  });
...
$('#multi_selection_toggle').click(function () {
  ...
  localStorage.setItem('lfm_multi_selection_enabled',multi_selection_enabled?'true':'false');
  ...
  $('#select_all_toggle').toggleClass('d-none', !multi_selection_enabled);
});
$('#select_all_toggle').click(function (){
  if(selected.length>1){
    ...
    selected = [];
    $('#content.list').children('a').each(function (index,item){
...
$(document).on('click', '[data-display]', function() {
  ...
  localStorage.setItem('lfm_show_list',show_list);
  loadItems();
});
...
function goTo(new_dir) {
  localStorage.setItem('lfm_working_dir',new_dir);
...
function loadFolders() {
  performLfmRequest('folders', {}, 'html')
    .done(function (data) {
      $('#tree').html(data);
      if(localStorage.getItem('lfm_working_dir')){
        $('#working_dir').val(localStorage.getItem('lfm_working_dir'));
      }
      if(localStorage.getItem('lfm_show_list')){
        show_list = localStorage.getItem('lfm_show_list');
      }
      if(localStorage.getItem('lfm_sort_type')){
        sort_type = localStorage.getItem('lfm_sort_type');
      }
      loadItems();
    });
}
